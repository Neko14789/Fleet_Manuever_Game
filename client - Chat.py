...
send_username_button = Button(Login, text="Send", command=lambda: send_username_button_press()).grid(row=1, column=1, sticky=S + E + W)
...
send_message_button = Button(master_window, text="Send", width=20, height=2, command=lambda button=master_window: send_message_button_press(button)).grid(row=1, column=3, sticky=N + S + E + W)
...
def Multiplayer_Update(message = ""):
        def update_message_box(UName, Mg):
            ...
            message_box.insert(END, "\n" + "<" + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ">-<" + UName + ">: " + Mg + "\n")
            message_box.see(END)
