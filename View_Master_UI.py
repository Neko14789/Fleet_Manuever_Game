...
class View:
    ...
    def __init__(self):
        ...
        def create_label_frame(row, column):
            ...
            for i in range(height_rows):  # Rows
                ...
                for j in range(width_columns):  # Columns
                    ...
                    l = Label(label_frame,cursor="dot",text="□■~",width=5,height=2)
                    #   Type Coordinate.py for Button Position ?
                    l.grid(row=i,column=j,sticky=N + S + E + W)

                    label_collection_set[(i,j)] = l
        ...
        testding = ttk.Separator(master).grid(column=1,row=1,rowspan=2,sticky=(S,E,N,E),ipadx=10)
